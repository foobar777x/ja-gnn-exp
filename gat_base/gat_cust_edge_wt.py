...
# Define a Heterogeneous GAT Model with Edge Weights
class HeteroGATWithEdgeWeights(nn.Module):
    def __init__(self, in_feats, hidden_feats, num_classes, edge_types, num_heads=4):
        super(HeteroGATWithEdgeWeights, self).__init__()
        assert hidden_feats % num_heads == 0, "hidden_feats must be divisible by num_heads"
        assert num_classes % num_heads == 0, "num_classes must be divisible by num_heads"
        self.conv1 = dgl.nn.HeteroGraphConv({
            etype: dgl.nn.GATConv(in_feats, hidden_feats // num_heads, num_heads, feat_drop=0.1, attn_drop=0.1)
            for etype in edge_types
        })
        self.conv2 = dgl.nn.HeteroGraphConv({
            etype: dgl.nn.GATConv(hidden_feats, num_classes // num_heads, num_heads, feat_drop=0.1, attn_drop=0.1)
            for etype in edge_types
        })
    ...
    def forward(self, g, x):
        ...
        x = {k: F.relu(v.mean(dim=1)) for k, v in x.items()}
        x = self.conv2(g, x, mod_kwargs={'edge_weight': edge_weights})
        x = {k: v.mean(dim=1) for k, v in x.items()}
        return x['review']

# Initialize the model
in_feats = features.shape[1]
num_heads = 1 # Adjust for the number of heads (4)
hidden_feats = 16 * num_heads  
num_classes = 2  # Binary classification (fraud or non-fraud)
model = HeteroGATWithEdgeWeights(in_feats, hidden_feats, num_classes, graph.etypes, num_heads)
